...
class TrainingCorpus:
    ...
    def strip_email_body(body):
        ...
        third_change = re.sub("http[s]?://\S+", '', second_change)  # delete all http/https links
        fourth_change = re.sub("www.\S+", '', third_change)  # delete all www links
        fifth_change = re.sub("[.+?]", '', fourth_change)  # delete everything in []
        sixth_change = ' '.join(re.sub(r'[^\w]', " ", fifth_change).split())  # delete all non alphanumeric symbols
        return sixth_change
    ...
    def compare_spam_and_ham_dicts(self):
        ...
        for spam_word in delete_spam:
            del (self.spam_dict[spam_word]) # delete all words that occur in ham and spam dictionaries
