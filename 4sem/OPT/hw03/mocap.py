...
'''
Assignment: https://cw.fel.cvut.cz/wiki/courses/b0b33opt/cviceni/hw/pca1/start?animal=wiki
'''
...
def playmotion(conn, A, B=None):
    ...
    conns = [x[x != 41]
             for x in np.split(conn, np.where(conn == 41)[0]) if len(x[x != 41])]
    ...
    if (B is not None):
        ...
        for conn in conns:
            macBs.append(ax.plot(B[0, conn, 0], B[1, conn, 0],
                         B[2, conn, 0], marker='o', color='r')[0])

    A = A.reshape(3, m//3, n, order='F')
    for conn in conns:
        macAs.append(ax.plot(A[0, conn, 0], A[1, conn, 0],
                     A[2, conn, 0], marker='o', color='b')[0])

    fig.legend(handles=[mpatches.Patch(
        color='red', label='approximation'), mpatches.Patch(color='blue', label='GT')])
    set_axes_equal(ax)
    ...
    ani = animation.FuncAnimation(fig, update_points, n, fargs=(
        A, B, macAs, macBs, conns), interval=1)
    plt.show()
