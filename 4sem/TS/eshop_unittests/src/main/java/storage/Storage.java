...
public class Storage {
     ...
     */
    public void insertItems(Item1 item, int count) {
     ...
     */
    public void removeItems(Item1 item, int count) throws NoItemInStorage {
        if (stock.containsKey(item.getID())) {
            ItemStock e = stock.get(item.getID());
            if (e.getCount() >= count) {
    ...
    public void processOrder(Order order) throws NoItemInStorage {
        ArrayList<Item1> items = order.getItems();
        for (Item1 i : items) {
            removeItems(i, 1);
        }
    }
     ...
     */
    public int getItemCount(Item1 item) {
        if (stock.containsKey(item.getID())) {
     ...
     */
    public Collection<Item1> getItemsOfCategorySortedByPrice(String category) {
        ArrayList<Item1> output = getItemsByCategory(category);
        this.sortItemsByPrice(output);
        return output;
    }


    private ArrayList<Item1> getItemsByCategory(String category) {
        ArrayList<Item1> output = new ArrayList();
        Collection<ItemStock> s = stock.values();
        for (ItemStock e : s) {
            Item1 tmp = e.getItem();
    ...
    private void sortItemsByPrice(ArrayList<Item1> items) {
        boolean sortedFlag = false;
        while (sortedFlag == false) {
            sortedFlag = true;
            for (int i = 0; i < items.size() - 1; i++) {
                Item1 current = items.get(i);
                Item1 next = items.get(i + 1);
